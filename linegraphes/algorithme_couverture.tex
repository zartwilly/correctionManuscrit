...
%Les ar\^etes et les sommets de $H$ et les ar\^etes et les sommets d\'ecouverts dans $G$
Les ar\^etes et les sommets de $H$ et $G$ peuvent avoir au cours de l'ex\'ecution plusieurs \'etats :
%''----------- sommets de H et G 
%\begin{itemize}
%	\item Sommets ``bien-d\'efinis'' : sommets d\'ecouverts de $G$ dont la clique correspondante dans $H$ a \'et\'e trouv\'e et identifi\'e.
%	\item Sommets ``\`a moiti\'e-nomm\'es'' :  sommets de $H$, dont  l'ar\^ete d\'ecouverte dans $G$, a une extr\'emit\'e  ``bien-d\'efinie''. 
%	%des sommets ``bien-d\'efinis'' de $G$ qui sont adjacents et dont l'ar\^ete qu'ils partagent dans $G$ est un sommet de $H$. 
%	\item Sommets ``pleinement-nomm\'es'' :  sommets de $H$, dont l'ar\^ete d\'ecouverte dans $G$  a des extr\'emit\'es, sont ``bien-d\'efinis''.
%	 %une ar\^ete de $G$ dont chaque extr\'emit\'e forme une clique dans $H$.
%	\item Sommets ``basiques'' : des ar\^etes de $G$ qui partagent une extr\'emit\'e. Ces sommets sont not\'es $x-y$ dans $H$ avec $x$ et $y$ des sommets d\'ecouverts de $G$.
%	\item Sommets ``partag\'es'' : sommets de $H$ partageant une ar\^ete avec des sommets ``basiques'' adjacents. Ce sont des ar\^etes d\'ecouvertes de $G$ qui ont une extr\'emit\'e commune. 
%\end{itemize}
\begin{itemize}
...
% ou les sommets partag\'es adjacents \`a deux sommets ``basiques'' qui forment une clique.
...
%L'algorithme de {\em couverture} est une am\'elioration de  celui de {\em Lehot} \cite{decompositionEnCliquesParArcs}.
...
%Concernant l'identification, il distingue cinq cat\'egories de sommets:
%\begin{itemize}
%\item les sommets ``bien-d\'efini" : des sommets du graphe racine $G$ dont leur clique correspondante dans $L(G)$ a \'et\'e trouv\'ee et identifi\'ee.
%\item les sommets ``\`a moiti\'e-nomm\'e" : des sommets de $L(G)$ dont un sommet de leur ar\^ete dans $G$ a d\'ej\`a \'et\'e ``bien-d\'efini".
%\item les sommets ``pleinement-nomm\'e" : des ar\^etes de $G$ dont leurs sommets dans $L(G)$ sont "bien-d\'efini".
%\item les sommets ``basique" : sommets adjacents dans $L(G)$. Not\'e $2-3$, chaque chiffre est un sommet dans $G$.
%\item les sommets ``d'intersection" : sommets adjacents aux sommets ``basique" appartenant \`a deux cliques.
%\end{itemize}
%Le but de cet algorithme est d'identifier les sommets ``d'intersection" parmi les sommets adjacents d'un sommet de $L(G)$ \`a partir des trois cas \cite{decompositionEnCliquesParArcs}. \newline
%L'algorithme d\'ebute par la s\'election de deux sommets ``basique" $1-2, 2-3$ et de leur ensemble d'adjacences $X$.
%Si $|X| = \emptyset$ alors tous les sommets des cliques sont identifi\'es en sommets ``\`a moiti\'e-nomm\'e".
%Si $|X| = 1$ alors il existe un sommet $x$ adjacent aux deux sommets ``basique" ($1-2,2-3$) tel que $(x,1-2,2-3)$ forme un triangle ``impair" si $x=2-4$ ou ce sommet forme un triangle ``pair" si $x=1-3$. Le sommet $x$ devient un sommet {\em d'intersection} et est identifi\'e en ``pleinement-nomm\'e". Les autres sommets $1-2,2-3$ sont identifi\'es en sommets ``\`a moiti\'e-nomm\'e".
%Dans le cas o\`u  $|X = \{x,y\}|=2$, si $x$ et $y$ sont adjacents alors il n'existe pas de sommets ``d'intersection" sinon ils forment deux triangles avec $1-2,2-3$. Un de ces triangles est ``impair" et il implique que $x=2-4$ , $y=1-3$. $y$ est identifi\'e en sommet {\em d'intersection}.
%Enfin pour $|X =\{ a,b,...\}| \ge 3$, le sommet $b$ est d\'eclar\'e sommet {\em d'intersection}  si $a$ et $b$ sont adjacents sinon  le sommet $a$ devient un sommet {\em d'intersection}. 
%La derni\`ere \'etape s\'electionne al\'eatoirement un sommet $u$ ``\`a moiti\'e-nomm\'e". Ce sommet est adjacent \`a un sommet $v$ ``pleinement-nomm\'e". Si ce sommet $v$ n'est pas d\'ej\`a couvert par une clique, alors il existe un sommet $z$ ``pleinement-nomm\'e" qui appartient d\'ej\`a \`a une clique et qui est d\'eclar\'e sommet {\em d'intersection} de ces deux pr\'ec\'edents sommets $u$ et $v$. Nous trouvons ces sommets $u$ et $v$ dans une pile dans laquelle $u$ et $v$ sont en t\^ete de pile et nous les nommons avec le num\'ero de la clique. 
%\`A la fin de cette \'etape, tous les sommets sont nomm\'es ``pleinement-nomm\'e" et nous identifions les sommets ``pleinement-nomm\'e" adjacents par un nouveau num\'ero de clique.
%%% ---- figure etapes de decouverte de l'algo de lehot
%\begin{figure}[htb!]
%	\centering
%	\includegraphics[scale=0.50]{deroulementAlgorithmeLehot.eps}\vspace{-0.5em}
...
%%% ---- figure etapes de decouverte de l'algo de lehot
%\newline
%Nous comparons le line-graphe propos\'e $H$ et le line-graphe $L(G)$ de $G$, ar\^etes par ar\^etes, et s'il existe une clique de $H$ n'ayant pas de correspondance dans $L(G)$ alors $L(G)$ n'est pas un line-graphe et l'algorithme ne retourne pas de r\'esultats c'est-\`a-dire aucunes cliques d\'ecouvertes lors de l'identification des sommets.
%Cet algorithme s'ex\'ecute en $O(N)+E$ avec $N$  le nombre d'ar\^etes dans $G$ et $E$ le nombre d'ar\^etes dans $L(G)$ car le processus d'identification des ar\^etes est en $O(N)$ et la comparaison entre les graphes est en $E$ \'etapes.
%\newline
%
%\subsubsection{Description de l'algorithme de couverture}
%Notre algorithme de couverture  couvre autant que possible des sommets du graphe de corr\'elation  $G_c$ par une ou deux cliques et si $G_c$ ne contient aucune case \'erronn\'ee alors il retourne sa {\em couverture de corr\'elation} ${\cal H}$. Dans le cas contraire, les sommets couverts par plus de deux cliques sont sauvegard\'es dans l'ensemble $\cal C$ des sommets \`a corriger. Notre algorithme de couverture retourne alors les cliques d\'ecouvertes et l'ensemble $\cal C$. 
%%La particularit\'e de notre algorithme est que les sommets appartenant \`a des cliques d\'ej\`a decouvertes ne sont plus trait\'es.
%
%Soit le graphe de corr\'elation $G_c = (V,E)$.
%Consid\'erons $\cal H$ un ensemble de cliques d\'ecouvertes de $G_c$ initialement vide et chaque sommet $v \in V$ a un \'etat  $Cliq(v)$ initialis\'e \`a $0$.
%\newline
%Chaque sommet $v$ a cinq \'etats possibles:
%\begin{itemize}
%	\item $0$ : il n'est couvert par aucune clique. Il correspond aussi \`a l'\'etat initial des sommets. C'est un sommet ``basique''.
%	\item $1$ : le sommet $v$ est couvert par une clique ou deux cliques. Dans le cas o\`u il est couvert par deux cliques, l'intersection de ces cliques donne le sommet $v$. Ce sommet est identifi\'e par ``pleinement-nomm\'e".
%	\item $2$ : le sommet est couvert par une clique et peut \^etre couvert par une seconde clique. Ce sommet est ``bien-nomm\'e".
%	\item $3$ : le sommet $v$ est un sommet ambigu. Ce sommet est ``d'intersection''.
%	\item $-1$ :  le sommet $v$ est couvert par plus de deux cliques. Il est contenu dans l'ensemble $\cal C$ et doit \^etre corrig\'e par l'algorithme de correction.
%\end{itemize}
%Nous choisissons un sommet $v$ de degr\'e minimum tel qu'il n'appartient \`a aucune clique ou qu'il est un sommet ambigu. 
%S'il existe une partition coh\'erente de ce sommet et de son voisinage $\{v\} \cup \Gamma_{G_c}(v)$ en deux cliques $C_1, C_2$, alors ces deux cliques sont contenues dans la line-couverture $\cal H$ en cours de construction. Les sommets $v$ et $u$ (avec $u \neq v$), appartenant \`a $C_1$ ou $C_2$, ont leurs \'etats modifi\'es de la mani\`ere suivante :
%\begin{itemize}
%\item $Cliq(v) = 1$ si son \'etat pr\'ec\'edent est \'egal \`a $0$ et la clique $C_2$ est vide. 
%\item $Cliq(v) = 3$ si son \'etat pr\'ec\'edent est \'egal \`a $0$ et la clique $C_2$ est non vide. 
%\item $Cliq(v) = 2$ si son \'etat pr\'ec\'edent est diff\'erent de $0$.
...
%\item $Cliq(u) = -1$ si son \'etat pr\'ec\'edent est \'egal \`a $3$ et l'ensemble des ar\^etes incidentes \`a $u$ est non vide.
%\end{itemize} 
%Dans le cas o\`u il n'existe aucune partition coh\'erente au sommet $v$, son \'etat est \`a $Cliq(v)=-1$.
%\newline
%
%%% description de l'algo
%L'algorithme de recherche de la couverture de corr\'elation $\cal H$ consid\`ere, tant qu'il en existe, un sommet dont les ar\^etes incidentes sont couverts par une seule clique. 
%Il labellise ce sommet en $Cliq = 1$, sauvegarde cette clique dans $\cal H$ puis supprime ces ar\^etes incidentes dans le graphe $G_c$. 
%Si au cours de l'ex\'ecution un tel sommet n'existe pas alors l'algorithme consid\`ere un sommet dont les ar\^etes incidentes peuvent \^etre couvertes par deux cliques et qui n'a pas \'et\'e pr\'ec\'edemment couvert par une clique de $H$. 
%Si cette partition en deux cliques est unique, l'algorithme labellise ce sommet en $Cliq = 1$ et supprime les ar\^etes. 
%Dans le cas o\`u ce sommet appartient \`a un des cas de la figure \ref{configurationAmbiguite} (cas d'un sommet encadr\'e), nous avons montr\'e que ces graphes sont les seuls line-graphes pour lesquels deux partitions possibles existent. 
%Nous utilisons la fonction de d\'ecision {\em Verif-correl} pour lever l'ambiguit\'e.
%Les autres sommets de la clique sont labellis\'es selon trois cas. Dans le premier cas, ils recoivent $Cliq \in \{2,3\}$ si ils poss\`edent des ar\^etes incidentes et leur label actuel est $Cliq \in \{ 3,0\}$. Dans le second cas, $Cliq = 1$ est attribu\'e \`a ces sommets si l'ensemble des ar\^etes incidentes est vide. Enfin, dans le dernier cas, ils recoivent $Cliq = -1$ si leur label actuel est  $Cliq \in \{2,3\}$ et ces ar\^etes incidentes ne forme pas une clique.
%\`A la fin  de l'ex\'ecution de cet algorithme, tous les sommets qui sont labellis\'es \`a $Cliq= -1$  sont \`a corriger par l'algorithme de correction. Ces sommets appartiennent \`a l'ensemble $\cal C$ des sommets \`a corriger.
%%Dans cet algorithme, on consid\'ere \`a chaque \'etape un sommet couvert par au plus une clique et dont toutes les ar\^etes incidentes ne sont pas (encore) couvertes par une des cliques pr\'ealablement choisie. On d\'etermine alors si cet ensemble d'ar\^etes appartient \`a une nouvelle clique (notamment dans le cas d'un sommet d\'ej\`a contenu par une clique), soit s'il est couvert par deux nouvelles cliques (uniquement dans le cas ou le sommet n'est couvert par aucune clique d\'ej\`a trouv\'ee). Les sommets correspondant au premier cas sont trait\'es en priorit\'e, et \`a chaque \'etape, si une possibilit\'e d'ambiguit\'e est d\'etect\'ee (figure \ref{configurationAmbiguite}), elle est lev\'ee par l'utilisation de la fonction \textit{Verif\_correl}.
%
%
%\begin{algorithm}[!ht]
%\caption{couverture\_cliques}
%\noindent DEBUT\\
%\noindent 1. {\bf Si} $G_c$ est isomorphe \`a un graphe double (voir figure \ref{graphe2Couverture} ), {\bf alors} le traiter avec Verif\_correl$(^1)$ \\
%~~\indent {\bf Sinon} \\
%~2. \indent {\bf Tant que} il existe un sommet $u$ t.q $Cliq(u) \in \{0,3\}$\\ 
%       	\indent~~~~~~{\bf Faire}\\
%~3.	       	\indent~~~~~~~~choisir $u$ de degr\'e minimum\\
...
%
%\FloatBarrier
%$^1$ : chaque graphe de la figure \ref{graphe2Couverture} admet deux line-couvertures, souvent isomorphes, mais une seule de ces line-couvertures correspond au DAG du r\'eseau \'electrique sous-jacent. Dans ce cas, on utilise les mesures de corr\'elation de la matrice de mesures $M_c$ afin de d\'eterminer la line-couverture la plus probable.
%\newline
%
% $^2$ :  le sommet $u$ choisi (s'il existe) ne sera pas prioritairement un sommet tel que $Cliq(u) = 0$ et $u$ est un point d'ambigu\"{i}t\'e. Si lors d'une \'etape, seul un tel choix est possible et qu'il n'y a aucun sommet $u$ tel que $Cliq(u) = -1$, c'est que chaque sommet du graphe initial $G_c$ est un point d'ambigu\"{i}t\'e.
% Dans ce cas, $G_c$ est une union de composantes connexes isomorphes \`a un des graphes de la figure  \ref{graphe2Couverture}.
%Dans ce cas, n'importe quel choix conduit \`a une d\'ecomposition correcte.
%Dans tous les autres cas de sommets $u$ s\'electionn\'es, le lemme \ref{lemmaCoherente} montre que le choix de d\'ecomposition est unique si $G_c$ est un line-graphe.
%\newline
%
%
%% parler d'ambiguite  
%% afin de leverr l'ambiguite, on a recous a u oracle dont la reponse est tjrs correcte. l'oracle utilse les lois de conversation .....
%
%\subsubsection{Complexit\'e de l'algorithme de couverture}
...
%\newline
%
%En conclusion, si le graphe $G_c$ est bien un line-graphe, tous ses sommets sont labellis\'es \`a $Cliq = 1$ et l'algorithme  de couverture trouvera une partition du voisinage d'un sommet en une ou deux cliques de fa\c con unique (voir les lemmes pr\'ec\'edents). 
%Une fois ce sommet et ses ar\^etes incidentes supprim\'es, le graphe restant est toujours un line-graphe, et la propri\'et\'e se propage.
%%Donc, si $G_c$ est un line-graphe, cet algorithme en trouvera toujours la couverture de corr\'elation unique.
%Ainsi $G_c$ qui poss\`ede des sommets labellis\'es \`a $Cliq = -1$ n'est pas un line-graphe. Nous allons proposer l'algorithme de correction qui retourne le line-graphe le plus proche de $G_c$.